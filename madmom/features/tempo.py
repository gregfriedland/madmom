...
from madmom.processors import BufferProcessor, OnlineProcessor
...
class CombFilterTempoEstimationProcessor(BaseTempoEstimationProcessor):
    ...
    def __init__(self, min_bpm=MIN_BPM, max_bpm=MAX_BPM, act_smooth=ACT_SMOOTH,
        ...
        self.alpha = alpha
        if self.online:
            self._out_buffer = BufferProcessor((np.max(self.intervals) + 1,
                                                len(self.intervals)))
    ...
    def interval_histogram_online(self, activations, reset=True):
        ...
        out = activations + self.alpha * self._out_buffer.buffer[idx]
        # shift output buffer with new value
        self._out_buffer(out)
        # determine the tau with the highest value
        act_max = out == np.max(out, axis=-1)[..., np.newaxis]
